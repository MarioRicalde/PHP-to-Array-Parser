...
class dBug {
	...
	function checkType($var) {
      ...
      echo '<table cellspacing=2 cellpadding=3 class="dBug_object">';
      echo '<tr>';
      echo '<td class="fieldrow"><pre>( ' . gettype($var) . ' )</pre> ' . $var . '</td>';
      echo '</tr>';
      echo '</table>';
				break;
		}
	...
	function varIsNULL() {
		echo '<table cellspacing=2 cellpadding=3 class="dBug_object">';
		echo '<tr>';
		echo '<td class="fieldrow">NULL</td>';
    echo '</tr>';
    echo '</table>';
	}
		...
		if(is_array($var)) {
			foreach($var as $key=>$value) {
				...
				if(in_array(gettype($value),$this->arrType))
					...
					echo ' <pre>(' . gettype($value) . ')</pre> ' . $value;
				}
	...
	function varIsObject($var) {
		...
		if(is_object($var)) {
			...
			foreach($arrObjVars as $key=>$value) {
				...
				if(in_array(gettype($value),$this->arrType))
					$this->checkType($value);
				else echo ' <pre>(' . gettype($value) . ')</pre> ' . $value;
				echo $this->closeTDRow();
			}
	...
	function initJSandCSS() {
        ...
        .fieldrow {background-color:#FFFFFF !important}
        pre {display:inline;}
			</style>
SCRIPTS;
	}
